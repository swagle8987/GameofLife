...
class Block:
    def __init__(self, x, y, ccolor, isAlive):
...
if __name__ == "__main__":
    ...
    for i in range(maxx):
        for j in range(maxy):   
            boxesnp[i,j]=Block(i*10,j*10,WHITE,False)
    ...
    """for i in range(n):
        if not ch:
            ...
            x1=random.randint(0,125)
            y1=random.randint(0,69)"""
    for x1,y1 in coords:
        print(x1,y1)
        boxesnp[x1,y1].alive=1
